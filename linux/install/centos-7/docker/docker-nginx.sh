...
# 判断是否有工作空间
echo "判断是否有工作空间: $DOCKER_WROKSPACE"
...
# 判断是否有默认配置文件
echo "判断是否有默认配置文件: $NGINX_WORKSPACE_CONF/nginx.conf"
...
echo "判断是否有同名容器: $CHECK_CONTAINER_NAME"

if [ -z "$CHECK_CONTAINER_NAME" ]; then
    echo "创建容器 $DOCKER_CONTAINER_NAME"
    # 创建容器
    docker run \
